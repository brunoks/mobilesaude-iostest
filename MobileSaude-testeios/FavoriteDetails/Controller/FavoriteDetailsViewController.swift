...
class FavoriteDetailsViewController: UIViewController {
    ...
    @objc fileprivate func saveFavorites() {
        ...
        if self.detailsView.favoritedButton.isFavorited {
            print("favorited")
            save.saveNewsFavorite(with: self.item)
    ...
    fileprivate func fetchDetails() {
        let save = SaveNews()
        let new = save.fetchFavorite(by: self.id)
        if let new = new {
            self.item = new.map( {return FavoriteDViewModel(model: $0)} ).first!
